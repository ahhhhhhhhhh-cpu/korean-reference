...
| romanization | text | 罗马字（可选，第一版不保证搜索） |
| irregular_type | text | 不规则类型（nullable） |
| hanja_text | text | 对应汉字（nullable） |
| status | text | `draft` / `published` / `archived` |
| frequency_level | text | 频率标签（nullable） |
| difficulty_level | text | 难度标签（nullable） |
...
| slug | text | URL 用 |
| name_ko | text | 韩文规则名 |
| category | text | 分类：连音、鼻音化、流音化… |
| difficulty | int | 难度 1–5（nullable） |
...
| target_form | text | 目标形式标识，如 `past_polite` |
| result_ko | text | 变形结果 |
...
| register | text | 语体：formal / informal / neutral |
| status | text | |
| created_at | timestamptz | |
| updated_at | timestamptz | |
| published_at | timestamptz | |
...
**relation_type 示例：** `related`, `confusable`, `synonym`, `see_also`
...
|------|------|------|
| id | uuid | PK |
| content_type | text | `entry` / `sound_rule` / `conjugation` / `hanja` / `idiom` / `page` |
| content_id | uuid | 关联内容 ID（nullable） |
| page_url | text | 提交时页面 URL |
| issue_type | text | `pronunciation` / `sound_change` / `conjugation` / `hanja` / `translation` / `example` / `display` / `other` |
...
| 实体 | 可搜索字段 |
|------|-----------|
| entries | headword_ko, headword_normalized, romanization, 各 locale definition |
| sound_rules | name_ko, 各 locale title/summary |
| hanja_entries | word_ko, hanja_text, 各 locale definition |
